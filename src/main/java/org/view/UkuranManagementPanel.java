...
public class UkuranManagementPanel extends JPanel {
    ...
    private List<Ukuran> initialList;
    ...
    private void initComponents() {
        ...
        JLabel lblTitle = new JLabel("Kelola Data Ukuran");
        ...
        txtSearch.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "Cari Nama Ukuran...");
        ...
        btnTambah = new JButton("Tambah Ukuran");
        ...
        String[] columns = {"No", "Nama Ukuran", "Tanggal Dibuat", "ID"};
    ...
    private void loadData() {
        initialList = ukuranDAO.findAll();
        displayData(initialList);
    }

    private void displayData(List<Ukuran> list) {
    ...
    private void filterData() {
        ...
        if (keyword.isEmpty()) {
            ...
            List<Ukuran> filtered = initialList.stream()
                    .filter(u -> u.getNamaUkuran().toLowerCase().contains(keyword))
    ...
    private void handleAdd() {
        ...
        UkuranFormDialog dialog = new UkuranFormDialog((Frame) parentWindow, null);
    ...
    private void handleEdit() {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Pilih ukuran yang ingin diedit!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int id = (int) table.getModel().getValueAt(table.convertRowIndexToModel(row), 3);
        Ukuran selected = initialList.stream().filter(u -> u.getId() == id).findFirst().orElse(null);
        
        if (selected != null) {
            Window parentWindow = SwingUtilities.getWindowAncestor(this);
            UkuranFormDialog dialog = new UkuranFormDialog((Frame) parentWindow, selected);
            dialog.setVisible(true);
            if (dialog.isSuccess()) {
                loadData();
            }
    ...
    private void handleDelete() {
        int row = table.getSelectedRow();
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Pilih ukuran yang ingin dihapus!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int id = (int) table.getModel().getValueAt(table.convertRowIndexToModel(row), 3);
        int confirm = JOptionPane.showConfirmDialog(this, "Apakah Anda yakin ingin menghapus ukuran ini?", "Konfirmasi Hapus", JOptionPane.YES_NO_OPTION);
        
        if (confirm == JOptionPane.YES_OPTION) {
            if (ukuranDAO.delete(id)) {
                loadData();
            }
        }
    }
