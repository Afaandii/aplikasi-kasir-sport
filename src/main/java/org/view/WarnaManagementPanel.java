...
public class WarnaManagementPanel extends JPanel {
    ...
    private List<Warna> initialList;
    ...
    private void initComponents() {
        ...
        JLabel lblTitle = new JLabel("Kelola Data Warna");
        ...
        txtSearch.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "Cari Nama Warna...");
        ...
        btnTambah = new JButton("Tambah Warna");
        ...
        String[] columns = {"No", "Nama Warna", "Tanggal Dibuat", "ID"};
    ...
    private void loadData() {
        initialList = warnaDAO.findAll();
        displayData(initialList);
    }

    private void displayData(List<Warna> list) {
    ...
    private void filterData() {
        ...
        if (keyword.isEmpty()) {
            ...
            List<Warna> filtered = initialList.stream()
                    .filter(w -> w.getNamaWarna().toLowerCase().contains(keyword))
    ...
    private void handleAdd() {
        ...
        WarnaFormDialog dialog = new WarnaFormDialog((Frame) parentWindow, null);
    ...
    private void handleEdit() {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Pilih warna yang ingin diedit!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int id = (int) table.getModel().getValueAt(table.convertRowIndexToModel(row), 3);
        Warna selected = initialList.stream().filter(w -> w.getId() == id).findFirst().orElse(null);
        
        if (selected != null) {
            Window parentWindow = SwingUtilities.getWindowAncestor(this);
            WarnaFormDialog dialog = new WarnaFormDialog((Frame) parentWindow, selected);
            dialog.setVisible(true);
            if (dialog.isSuccess()) {
                loadData();
            }
    ...
    private void handleDelete() {
        int row = table.getSelectedRow();
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Pilih warna yang ingin dihapus!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int id = (int) table.getModel().getValueAt(table.convertRowIndexToModel(row), 3);
        int confirm = JOptionPane.showConfirmDialog(this, "Apakah Anda yakin ingin menghapus warna ini?", "Konfirmasi Hapus", JOptionPane.YES_NO_OPTION);
        
        if (confirm == JOptionPane.YES_OPTION) {
            if (warnaDAO.delete(id)) {
                loadData();
            }
        }
    }
