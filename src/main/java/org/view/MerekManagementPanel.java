...
public class MerekManagementPanel extends JPanel {
    ...
    private List<Merek> initialList;

    public MerekManagementPanel() {
    ...
    private void initComponents() {
        ...
        JLabel lblTitle = new JLabel("Kelola Data Merek");
        ...
        txtSearch.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "Cari Nama Merek...");
        ...
        btnTambah = new JButton("Tambah Merek");
        ...
        String[] columns = {"No", "Nama Merek", "Tanggal Dibuat", "ID"};
    ...
    private void loadData() {
        initialList = merekDAO.findAll();
        displayData(initialList);
    }

    private void displayData(List<Merek> list) {
    ...
    private void filterData() {
        ...
        if (keyword.isEmpty()) {
            ...
            List<Merek> filtered = initialList.stream()
                    .filter(m -> m.getNamaMerek().toLowerCase().contains(keyword))
    ...
    private void handleAdd() {
        ...
        MerekFormDialog dialog = new MerekFormDialog((Frame) parentWindow, null);
    ...
    private void handleEdit() {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Pilih merek yang ingin diedit!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int id = (int) table.getModel().getValueAt(table.convertRowIndexToModel(row), 3);
        Merek selected = initialList.stream().filter(m -> m.getId() == id).findFirst().orElse(null);
        
        if (selected != null) {
            Window parentWindow = SwingUtilities.getWindowAncestor(this);
            MerekFormDialog dialog = new MerekFormDialog((Frame) parentWindow, selected);
            dialog.setVisible(true);
            if (dialog.isSuccess()) {
                loadData();
            }
    ...
    private void handleDelete() {
        int row = table.getSelectedRow();
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Pilih merek yang ingin dihapus!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int id = (int) table.getModel().getValueAt(table.convertRowIndexToModel(row), 3);
        int confirm = JOptionPane.showConfirmDialog(this, "Apakah Anda yakin ingin menghapus merek ini?", "Konfirmasi Hapus", JOptionPane.YES_NO_OPTION);
        
        if (confirm == JOptionPane.YES_OPTION) {
            if (merekDAO.delete(id)) {
                loadData();
            }
        }
    }
